...
#let authors_and_affiliations() = {
  set text(
    ...
    size: SIZE1.get())
  let content = text(AUTHORS.get().at(0).at(0) + "")
  for index in range(1, AUTHORS.get().len()) {
    let author = AUTHORS.get().at(index)
    let name = author.at(0)
    let affiliation = author.at(1)
    content += h(0.2em) + text(" | ") + h(0.25em) + text(name + "")
  }
  content
}
#let caption_title_page() = {
  set text(
    fill: COLOUR_PALETTE.get().at("background"),
    size: SIZE1.get())
  let content = text(AUTHORS.get().at(0).at(0) + " at " + AUTHORS.get().at(0).at(1))
  for index in range(1, AUTHORS.get().len()) {
    let author = AUTHORS.get().at(index)
    let name = author.at(0)
    let affiliation = author.at(1)
    content += h(0.2em) + text(" | ") + h(0.25em) + text(name + " at " + affiliation)
  }
  content
}
...
#let title_and_section(section, subsection) = {
  context {
    set text(
      ...
      size: SIZE0.get())
    text(section)
    place(horizon + right, text(subsection, fill: COLOUR_PALETTE.get().at("background")))
  }
}
...
#let supervisors() = {
  set text(
    fill: COLOUR_PALETTE.get().at("background"),
    size: SIZE1.get())
  let content = text("Examiners: Examiner 1, Examiner 2")
...
#let generate_slide(section, subsection, descriptor, title: none) = {
  SLIDE_COUNTER.step()
  let final_descriptor = if title != none {
    componentwise_add_descriptors(
      descriptor,
      Place(top + center, lift(
...
#let generate_slide2(section, subsection, descriptor, title: none) = {
  let final_descriptor = if title != none {
    componentwise_add_descriptors(
      descriptor,
      Place(top + center, lift(
